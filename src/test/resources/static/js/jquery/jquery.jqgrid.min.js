	...
	return this.each( function() {
		...
		var ts= this, grid={
			...
			populateVisible: function() {
				...
				console.log("st", scrollTop, "tpt", table.position().top, "tcss", $.css(table[0], "top") );
				...
				if (npage) {
					...
					if (grid.hDiv.loading) {
						...
						p.page = page;
						console.log(p.page, empty, ttop, npage, p.reccount);
						if(!empty && p.reccount > 150) {
							empty = true;
						}
						if (empty) {
...
$.jgrid.extend({ //sennet : jqgrid 함수들
...
$.jgrid.extend({
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			//sennet.ksh : viewDialog modal open
			if($('.ui-jqgrid-title').text() == 'Translog'){
				var seq = $($t).find('#' + rowid + ':eq(0)>td:eq(0)').text();
				showCustomModal(seq);
			} else {
				if($('.ui-jqdialog-title:contains("System Status")').length > 0){
					$('#ViewGrid_list tr[id*="trv_host"]').each(function(a, b){
						$(b).find('span').each(function(c, d){
							if($(d).html() != '&nbsp;'){
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.view) {
					...
					.click(function(){//sennet : 상세보기 버튼 클릭 핸들러
						if (!$(this).hasClass( commonstyle.disabled )) {
							var sr = $t.p.selrow;
							if (sr) {
...
function showCustomModal(seq){
	$('#btnRefresh').unbind('click');
	$('#btnRefresh').on('click', function(){
		translogRefresh(seq);
	});
	location.href = '#popupTranslog';
	translogRefresh(seq);
}

//sennet.ksh : translog 새로고침 함수
function translogRefresh(seq){
	var url = document.location.href;
	var requestUrl = url.substring(0, url.indexOf('/admin'));
	$.ajax({
		url: requestUrl + '/rest/translog/' + seq,
		dataType: 'json',
		success: function(refreshData){
			var data = [refreshData.seq,
			            refreshData.name,
			            refreshData.taskid,
			            refreshData.code,
			            refreshData.error_message,
			            refreshData.from_host_seq,
			            refreshData.to_host_seq,
			            refreshData.from_file,
			            refreshData.to_file,
			            refreshData.trans_rate,
			            refreshData.startdate,
			            refreshData.enddate,
			            refreshData.interval,
			            refreshData.data_size,
			            refreshData.trans_size,
			            refreshData.send_script_result,
			            refreshData.send_script_feasibility,
			            refreshData.recv_script_result,
			            refreshData.recv_script_feasibility];
			if(refreshData.code == '00000'){
				$('#tableTranslog tr:eq(3)>td:eq(1)').css('background-color', 'red');
			} else {
				$('#tableTranslog tr:eq(3)>td:eq(1)').css('background-color', '');
			}
			$('#tableTranslog tr').each(function(i){
